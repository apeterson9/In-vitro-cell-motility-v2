...
% INPUT: expData structure with 7 fields
...
for iDir = 1:length(dirs) % For each directory (movie)
    ...
    % Get path to Imaris files, called 'movie_path'
    dirIdx = [];
    ...
    if ~isempty(expFile) % Make sure data isn't missing
        ...
        load([movie_path expFile]); % load as rawData structure with each 
                                   % fields (s#) corresponding to different
                                   % well on single device
        ...
        for iWell = 1:length(fields) % for every well
            %%try
                ...
                allData = rawData.(rep).ImarisData; % rawData table for single well
                % holds position data for each trackID at every single time
                % for a single well
                
                % construct a storage cell "IMG" that stores the image
                % matrix inside each cell
                v_obj = VideoReader([movie_path rep '.avi']);
                numFrames = v_obj.NumFrames;
                IMS = cell(numFrames,1);

                for t = 1:numFrames % populate IMG cell with each frame
                      frame = read(v_obj,t);
                      IMS{t,1} = frame(:,:,3);
                end

                % get inputs for vis_Imaris_tracks function
                uis = expData.ui{:,1};
                sfw_row = strcmp(uis(:),'Smoothing Frame Window') == 1;
                sfw = expData.ui{sfw_row,2}{1}; % smoothing frame window (sfw) input value

                mtmo_row = strcmp(uis(:),'Make Tracks Movie Option') == 1;
                mtmo = expData.ui{mtmo_row,2}{1}; % make tracks movie option (mtmo) input value

                ps_row = strcmp(uis(:),'Pixel Size') == 1;
                ps = expData.ui{ps_row,2}{1}; % pixel size (ps) input value
...
% %                 try
                    ...
                    [cellMeans, allTracks] = calculate_CI_in_vitro(allData,source,min_track_length);
                    % allTracks is table
...
%     end % if is-empty
                ...
                %%try
                    ...
                    for iCell = 1:length(cellIDs) % for every cell
                        ...
                        track_i = allData(idx,:);
                        class_out = nan(size(track_i,1), 3); % pre-allocating 3 cols for every time
                        if length(track_i(:,1)) > min_track_length % arbitrarily set to exclude shorter track fragments from visualization
                            ...
                            % plot x,y position
                            plot(track_i(:,3),track_i(:,4),'k','LineWidth',1); % track initially plotted in black
                            xlabel('x')
                            ylabel('y')
                            
                            % ,'Parent',ah{num} recently deleted 4.13.22
                            hold on
                            ...
                            for iClass = 1:numClasses
                                ...
                                if cellClass(iClass,3)>0
                                    ...
                                    plot(track_i(startFrame:endFrame,3),track_i(startFrame:endFrame,4),...
                                        'Color',cCode(cellClass(iClass,3),:),'LineWidth',1);
                                    title(rep)
                                    catch
                    ...
                    allTracks = [allTracks, classes]; % add class, diff_coeff, con_rad to each track time point
...
% %                     expData.statusTracker{5,4} = warnNum5;
                ...
                %%try
                    vel_by_class = speed_by_class(allTracks,cellMeans);
                    cellMeans = [cellMeans, vel_by_class];
                    rawData.(rep).chemotaxisStats = allTracks;
...
% %                 end
                ...
                clear allData % this is input data (track, t, x, y, z, speed
