...
# Right now this is an optional block-I think the user can decide if they want to run the same cleanup script on all data sets and take out the aggregating section in the cleanup script, or they can aggregate here. 
...
bee.plot <- ggplot(agg, aes(x = cell_line, y = num_pauses)) +
  ...
  labs(
    ...
    title = "Figure ##",
    subtitle = "Arpc1b KO cells pause a greater number of times than control cells"
  ) 

wilcox.test(data_clean$num_pauses~data_clean$cell_line)
...
wilcox.test(data_clean$pause_duration~data_clean$cell_line)
...
#wilcox.test(agg$pause_duration~agg$cell_line)
bee.plot
...
```{r, echo = TRUE}
# Define mixed-effects model
mixed.speed <- lmer( num_pauses ~ cell_line*stimulus + (1|f.day) + 
                       (1|f.day:slide), data=agg, REML=T)
tab_model(mixed.speed, show.re.var = TRUE, show.r2 = FALSE)
```