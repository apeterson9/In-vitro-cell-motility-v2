...
* __Chemotactic Index__: Calculated using methods described by Kim and Hayne (2013). $CI = \frac{x}{d_{TOT}}$; where $x$ is the final displacement along the direction of the gradient, and $d_{TOT}$ is the total migration distance. 
...
The mean speed for the control cells was subtracted from the mean cell speed for the KO cells resulting in a negative value. This will be more intuitive for plotting later. To obtain the estimated mean and confidence intervals for the effect size, a one-sample t-test was used. 
...
( tt <- with(wksp, tapply( I(mean_velocity/J*60), list(stimulus, cell_line, f.day), mean, na.rm=TRUE))) 
...
```{r Track Length, message = FALSE, warning = FALSE, echo = FALSE, results = FALSE}
mod_out <- lmer( track_length ~ cell_line*stimulus + (1|f.day) + (1|f.day:slide) + (1 |f.day:trt), 
            data=agg, REML=T)
 
( tt <- with(wksp, tapply( track_length, list(stimulus, cell_line, f.day), mean, na.rm=TRUE))) 
( tt <- apply(tt, MARGIN=c(3,2), mean, na.rm=TRUE)) # average across stimuli to get one mean speed per experimental day
tt <- tt[!is.na(tt[,1]), ]

effect_track_length <- data.frame(value = tt[,2] - tt[,1]) %>%
  mutate(metric = "Track Length") 
cis <- t.test((tt[,2] - tt[,1])/tt[,1])
temp <- data.frame(cbind(metric = "Track Length", estimate = cis$estimate, Lower = cis$conf.int[1], Upper = cis$conf.int[2]))
...
```{r effect_distance, message = FALSE, warning = FALSE, echo = FALSE, results = FALSE}
mod_out <- lmer( cumul_distance ~ cell_line*stimulus + (1|f.day) + (1|f.day:slide) + (1 |f.day:trt), 
            data=agg, REML=T)
effect_sspeed = data.frame(day)
( tt <- with(wksp, tapply( cumul_distance, list(stimulus, cell_line, f.day), mean, na.rm=TRUE ))) 
( tt <- apply(tt, MARGIN=c(3,2), mean, na.rm=TRUE)) # average across stimuli to get one mean speed per experimental day
tt <- tt[!is.na(tt[,1]), ]
effect_cumul_distance <- data.frame(value = tt[,2] - tt[,1])%>%
  mutate(metric = "Cumulative Distance") 
cis <- t.test((tt[,2] - tt[,1])/tt[,1])
temp <- data.frame(cbind(metric = "Cumulative Distance", estimate = cis$estimate, Lower = cis$conf.int[1], Upper = cis$conf.int[2]))
cis_df <- rbind(cis_df, temp)
```
...
```{r net displacement, message = FALSE, warning = FALSE, echo = FALSE, results = FALSE}
mod_out <- lmer( net_displacement ~ cell_line*stimulus + (1|f.day) + (1|f.day:slide) + (1 |f.day:trt), 
            data=agg, REML=T)

( tt <- with(wksp, tapply( net_displacement, list(stimulus, cell_line, f.day), mean ))) 
( tt <- apply(tt, MARGIN=c(3,2), mean, na.rm=TRUE)) # average across stimuli to get one mean speed per experimental day

tt <- tt[!is.na(tt[,1]), ]

effect_displace <- data.frame(value = tt[,2] - tt[,1]) %>%
  mutate(metric = "Net Displacement")

cis <- t.test((tt[,2] - tt[,1])/tt[,1])
temp <- data.frame(cbind(metric = "Net Displacement", estimate = cis$estimate, Lower = cis$conf.int[1], Upper = cis$conf.int[2]))
cis_df <- rbind(cis_df, temp)
```
...
```{r effect_tortuosity, message = FALSE, warning = FALSE, echo = FALSE, results = FALSE}
mod_out <- lmer( tortuosity ~ cell_line*stimulus + (1|f.day) + (1|f.day:slide) + (1 |f.day:trt), 
            data=agg, REML=T)
 
( tt <- with(wksp, tapply( tortuosity, list(stimulus, cell_line, f.day), mean ))) 
( tt <- apply(tt, MARGIN=c(3,2), mean, na.rm=TRUE)) # average across stimuli to get one mean speed per experimental day
tt <- tt[!is.na(tt[,1]), ]

effect_tortuosity <- data.frame(value = tt[,2] - tt[,1]) %>%
  mutate(metric = "Tortuosity")

cis <- t.test((tt[,2] - tt[,1])/tt[,1])
temp <- data.frame(cbind(metric = "Tortuosity", estimate = cis$estimate, Lower = cis$conf.int[1], Upper = cis$conf.int[2]))
cis_df <- rbind(cis_df, temp)
```
...
```{r effect_chemotactic_index, message = FALSE, warning = FALSE, echo = FALSE, results = FALSE}
mod_out <- lmer( mean_CI ~ cell_line*stimulus + (1|f.day) + (1|f.day:slide) + (1 |f.day:trt), 
            data=agg, REML=T)
 
( tt <- with(wksp, tapply( mean_CI , list(stimulus, cell_line, f.day), mean, na.rm = TRUE ))) 
( tt <- apply(tt, MARGIN=c(3,2), mean, na.rm=TRUE)) # average across stimuli to get one mean speed per experimental day
tt <- tt[!is.na(tt[,1]), ]

effect_ci <- data.frame(value = tt[,2] - tt[,1]) %>%
  mutate(metric = "Chemotactic Index ") 

cis <- t.test((tt[,2] - tt[,1])/tt[,1])
temp <- data.frame(cbind(metric = "Chemotactic Index ", estimate = cis$estimate, Lower = cis$conf.int[1], Upper = cis$conf.int[2]))
cis_df <- rbind(cis_df, temp)
```
...
```{r effect_confinement radius, message = FALSE, warning = FALSE, echo = FALSE, results = FALSE}
...
effect_cr <- data.frame(value = tt[,2] - tt[,1]) %>%
  mutate(metric = "Confinement Radius") 

cis <- t.test((tt[,2] - tt[,1])/tt[,1])
temp <- data.frame(cbind(metric = "Confinement Radius", estimate = cis$estimate, Lower = cis$conf.int[1], Upper = cis$conf.int[2]))
cis_df <- rbind(cis_df, temp)
```
```{r, echo = FALSE, results = TRUE}
drop1(mod_out, ddf="Kenward-Roger")
...
```{r effect_mean_theta, message = FALSE, warning = FALSE, echo = FALSE, results = FALSE}
mod_out <- lmer( mean_theta ~ cell_line*stimulus + (1|f.day) + (1|f.day:slide) + (1 |f.day:trt), 
            data=agg, REML=T)
 
( tt <- with(wksp, tapply( mean_theta , list(stimulus, cell_line, f.day), mean, na.rm = TRUE ))) 
( tt <- apply(tt, MARGIN=c(3,2), mean, na.rm=TRUE)) # average across stimuli to get one mean speed per experimental day
tt <- tt[!is.na(tt[,1]), ]

effect_mean_theta <- data.frame(value = tt[,2] - tt[,1]) %>%
  mutate(metric = "Mean Theta") 

cis <- t.test((tt[,2] - tt[,1])/tt[,1])
temp <- data.frame(cbind(metric = "Mean Theta", estimate = cis$estimate, Lower = cis$conf.int[1], Upper = cis$conf.int[2]))
cis_df <- rbind(cis_df, temp)
```
```{r, echo = FALSE, results = TRUE}
drop1(mod_out, ddf="Kenward-Roger")
...
## TEST OUT NUMBER OF PAUSES USING THE LAYOUT OF THE DAY TO DAY VARIABILITY PLOT-use this for supplement
```{r num_pauses scatter supplemental figure, echo = FALSE, warning = FALSE,  fig.path='figures/', dev=c('png', 'svg')}
# Generate figure
scatter.plot <- ggplot(data_clean, aes(x = cell_line, y = num_pauses, colour = cell_line)) +
  geom_jitter(alpha = 0.3) +
  scale_colour_manual(values = c("#c51b7d","#4d9221")) +
  #scale_colour_manual(values = c("#f1a340", "#998ec3")) +
  facet_grid(~f.day) +
  
  stat_summary(fun.y = "mean", shape = 8, geom = "point", size = 3, color = "black") +
  ...
  labs(
    ...
    y = "Number of pauses",
    title = "Figure ##",
    subtitle = "Arpc1b KO cells pause more often than control cells",
    fill = "cell line"
  ) + 
  theme_Briana() +
  theme(
...
## GENERATE A SCATTER PLOT WITH THEME OF THE BOXPLOT GRAPHS, BUT AS SCATTER
```{r num_pauses scatter supplemental figure, echo = FALSE, warning = FALSE,  fig.path='figures/', dev=c('png', 'svg')}
# Generate figure
scatter.plot <- ggplot(data_clean, aes(x = cell_line, y = num_pauses, colour = cell_line)) +
  geom_jitter(alpha = 0.3) +
  ...
  labs(
    x = "Experimental Day",
    y = "Number of pauses (norm. to track length)",
    title = "Figure ##",
    subtitle = "Arpc1b KO cells pause more often than control cells",
    fill = "cell line"
  ) + 
...
bee.plot <- ggplot(data_clean, aes(x = cell_line, y = num_pauses)) +
  ...
  geom_boxplot(outlier.shape=NA, fill="#B3B3B3") + 
  geom_beeswarm(cex=3, aes(colour=f.day)) +
  geom_beeswarm(data=num_pauses_rep_averages, size=8) +
  scale_colour_brewer(palette = "PiYG") +
  #stat_summary(fun.y = "mean", shape = 8, size = 0.5, color = "black") +
  stat_summary(fun.data = "mean_sdl", fun.args = list(mult=1),
               geom="errorbar", color="black", width=0.5) + 
...
Due to the nature of the metric, pause duration is especially susceptible to skew by outliers. Here we perform additional filtering of outliers of pause duration based on 2 standard deviations away from the mean for the entire data set. 
```{r pause_duration filter outliers, echo = FALSE, warning = FALSE,  fig.path='figures/', dev=c('png', 'svg')}
# this section seems to filter out 559 data points (cells) just based on pausing duration
data_pause_dur_sum = data_clean %>%
  group_by(cell_line, f.day, stimulus) %>%
  summarise(duration_mean = mean(pause_duration),
            duration_sd = sd(pause_duration)) %>%
  ungroup() %>%
  mutate(pause_dur_high = duration_mean + (2 * duration_sd)) 

data_clean_pausing = data_clean %>%
  inner_join(data_pause_dur_sum) %>%
  filter(pause_duration < pause_dur_high) 
rm(data_pause_dur_sum)
```
## TESTING OUT THESE SCATTER PLOTS, NOW FOR PAUSE DURATION
```{r pause_duration scatter supplemental figure, echo = FALSE, warning = FALSE,  fig.path='figures/', dev=c('png', 'svg')}
# Generate figure
scatter.plot <- ggplot(data_clean_pausing, aes(x = cell_line, y = pause_duration, colour = cell_line)) +
  geom_jitter(alpha = 0.3) +
  scale_colour_manual(values = c("#c51b7d","#4d9221")) +
  #scale_colour_manual(values = c("#f1a340", "#998ec3")) +
  facet_grid(~f.day) +
...
bee.plot <- ggplot(data_clean, aes(x = cell_line, y = pause_duration)) +
  ...
  geom_jitter(cex=3, size=0.5, aes(colour=f.day)) +
  geom_beeswarm(data=agg, size=2, aes(x = cell_line, y = pause_duration, colour=f.day)) +
...
```{r num_pauses_box,  echo = FALSE, fig.path='figures/', dev=c('png', 'svg')}

#data <- subset(data_clean, data_clean$stimulus != "PBS") # load data organized and cleaned in previous step
# use data_clean for this step
# Generate figure
bee.plot <- ggplot(agg, aes(x = cell_line, y = num_pauses)) +
  facet_wrap(~stimulus) +
  geom_boxplot(outlier.shape=NA, fill="#B3B3B3") + 
  geom_beeswarm(cex=3, aes(colour=f.day)) +
  scale_colour_brewer(palette = "PiYG") +
...
# use data_clean for this step
# Generate figure
bee.plot <- ggplot(agg, aes(x = cell_line, y = pause_duration)) +
  facet_wrap(~stimulus) +
  geom_boxplot(outlier.shape=NA, fill="#B3B3B3") + 
  geom_beeswarm(cex=3, aes(colour=f.day)) +
  scale_colour_brewer(palette = "PiYG") +
  ...
  labs(
    ...
    y = "Pause duration",
    title = "Figure ##",
    subtitle = "Arpc1b KO cells pause for longer than control cells"
  ) 
bee.plot
```
...
##Calculate effect size for the pausing phenotype
```{r effect_num_pauses, message = FALSE, warning = FALSE, echo = FALSE, results = FALSE}

mod_out <- lmer( num_pauses ~ cell_line*stimulus + (1|f.day) + (1|f.day:slide) + (1 |f.day:trt), 
            data=agg, REML=T)
 
( tt <- with(wksp, tapply( num_pauses , list(stimulus, cell_line, f.day), mean, na.rm = TRUE ))) 
( tt <- apply(tt, MARGIN=c(3,2), mean, na.rm=TRUE)) # average across stimuli to get one mean speed per experimental day
tt <- tt[!is.na(tt[,1]), ]

effect_num_pauses <- data.frame(value = tt[,2] - tt[,1]) %>%
  mutate(metric = "Number of Pauses") 

cis_pause <- t.test((tt[,2] - tt[,1])/tt[,1])
cis_pause_df <- data.frame(cbind(metric = "Number of Pauses", estimate = cis_pause$estimate, Lower = cis_pause$conf.int[1], Upper = cis_pause$conf.int[2]))
...
```{r effect_pause_duration, message = FALSE, warning = FALSE, echo = FALSE, results = FALSE}
mod_out <- lmer( pause_duration ~ cell_line*stimulus + (1|f.day) + (1|f.day:slide) + (1 |f.day:trt), 
            data=agg, REML=T)
 
( tt <- with(wksp, tapply( pause_duration , list(stimulus, cell_line, f.day), mean, na.rm = TRUE ))) 
( tt <- apply(tt, MARGIN=c(3,2), mean, na.rm=TRUE)) # average across stimuli to get one mean speed per experimental day
tt <- tt[!is.na(tt[,1]), ]

effect_pause_duration <- data.frame(value = tt[,2] - tt[,1]) %>%
  mutate(metric = "Pause Duration") 

cis_pause <- t.test((tt[,2] - tt[,1])/tt[,1])
temp <- data.frame(cbind(metric = "Pause Duration", estimate = cis_pause$estimate, Lower = cis_pause$conf.int[1], Upper = cis_pause$conf.int[2]))
cis_pause_df <- rbind(cis_pause_df, temp)
```
...
pairs( emmeans(mod_out, "cell_line") ) 
```
## Plot additional effect sizes for pausing
```{r effect sizes, message = FALSE, warning = FALSE, fig.path='figures/', dev=c('png', 'svg')}

# Figure setup
# Convert cis_df$estimate and conf ints to numeric
cis_pause_df$estimate <- as.numeric(cis_pause_df$estimate)
cis_pause_df$Upper <- as.numeric(cis_pause_df$Upper)
cis_pause_df$Lower <- as.numeric(cis_pause_df$Lower)

# Print results to a table
effect_tab = cbind(cis_pause_df$metric, cis_pause_df$estimate, cis_pause_df$Lower, cis_pause_df$Upper) 
...
# Initialize plot
effect_plot <- ggplot(cis_pause_df, aes(x = 1, y = I(estimate))) +
  geom_linerange(aes(ymin = I(Lower), ymax = I(Upper), size = 2)) +
  geom_hline(yintercept = 0) +
  facet_wrap(~metric) + 
...
## Make a plot to show any differences in free, confined velocity
```{r lmer pause_duration, echo = TRUE}
