...
heads = ["day","slide","position","cell_line","stimulus","id","mean_speed"...
    ,"DI","cumulative_distance","CI","accuracy","MI","CR","num_pauses","pause_duration","free_vel","super_vel"];
T = table(nan,nan,{'pos'},{'cellLine'},{'stim'},nan,nan,nan,nan,nan,nan,nan,nan,nan,nan,nan,nan,'VariableNames',heads);
...
for iGroup = 1:length(Groups) % for every Group
    ...
    for iDir = 1:numDays% for every replicate
        ...
        for iSlide = 1:length(stats.(Groups{iGroup}).cellMeans.(days{iDir}))
            ...
            cellIDs = unique(scData.TrackID,'stable');
            ...
            for iCell = 1:length(cellIDs)
                ...
                cellIdx = find(scData.TrackID == cellIDs(iCell));
                ...
                T{tEnd+iCell,6}  = id;
                T{tEnd+iCell,7}  = meanCellDat.mean_velocity;
                T{tEnd+iCell,8}  = meanCellDat.DI;
                T{tEnd+iCell,9}  = meanCellDat.cumulative_distance;
                T{tEnd+iCell,10}  = meanCellDat.CI_Haynes;
                T{tEnd+iCell,11}  = meanCellDat.cos_theta;
                T{tEnd+iCell,12}  = meanCellDat.MI;
                T{tEnd+iCell,13}  = nanmean(track_i.confinement_radius);
                T{tEnd+iCell,14}  = meanCellDat.numPauses;
                T{tEnd+iCell,15}  = meanCellDat.duration;
                T{tEnd+iCell,16}  = meanCellDat.mean_vel_free;
                T{tEnd+iCell,17}  = meanCellDat.mean_vel_super;
